...
describe("test utils for Components", (): void => {
    describe("test entriesNotIn", (): void => {
        ...
        it("should be test entriesNotIn return header with filter", () => {
            expect.assertions(1);
            const headers: Record<string, string> = {name: "Nom", description: "Description", filter: "ToFilter"};
            const data = Object
                .entries(headers)
    ...
    describe("test objectByString", (): void => {
        it("should be have a result without search", () => {
            expect.assertions(5);
            const rows = [{name: "Product 1", description: "Description of product one"}, {name: "Product 2", description: "Description of product two"}];
